...
At national scale we ask for one of two data routes: (a) a daily or weekly feed of per-pub menu PDFs or JSON, or (b) continued access to the publicly posted menu PDFs with explicit written permission. A name licence covers the Wetherspoon mark and pub names in-game. Optional: a feature slot inside the Order and Pay app — "order, then play" — to reinforce the order-linked loop. Our support model: automated pipeline monitoring, a shared channel for pub managers, 48-hour SLA on data or gameplay issues during rollout.

## 7. Commercial model

Three options, and we will negotiate any of them:
...
Nominate one pilot pub and a single point of contact, and we will have the branded trial live within **72 hours**. Questions and trial approvals: **[Name, role, email, phone]**.
